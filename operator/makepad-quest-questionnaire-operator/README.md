...
cargo run --manifest-path operator\makepad-quest-questionnaire-operator\Cargo.toml --bin quest-questionnaire-operator-cli -- stop-session --session-id target-session-001 --protocol-version target.runtime.operator.v1 --runtime-kind unity_quest_apk --endpoint http://127.0.0.1:8787 --audit-dir artifacts\operator-audit
...
Pass `--verify-bundle` to `pull-target-session` to run the local verifier
immediately after the ADB copy. The verifier assumes the remote path names a
concrete session folder and checks `<out>\<remote-folder-name>`; pass
`--bundle-path` when the pulled layout is different. You can also run
