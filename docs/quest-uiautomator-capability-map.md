...
| `capture.media_projection.record_mp4` | Record a lab MP4 through app-owned MediaProjection | Temporarily grant `PROJECT_MEDIA` or use visible consent; launch `ProjectionRecordingActivity`; foreground service records H.264 MP4; restore app-op | Produces flat 2D capture under the automation app external-files recording directory; status JSON records start/stop/error state | Working lab route; observed encoder may deliver lower FPS/bitrate than requested |
...
| `quest.capture.brb_comparison_matrix` | Compare BRB/panel capture routes on the same short stimulus | `tools/Run-BrbRecordingComparison.ps1`; captures Metacam landscape attempt, Metacam portrait attempt, ADB `screenrecord`, and app-owned MediaProjection; records ffprobe metadata | ADB and MediaProjection produced usable short comparison clips. Back-to-back Metacam attempts exposed a file-finalization/state limitation and should not be treated as independent rows until isolated. | Partial; diagnostic matrix working |
